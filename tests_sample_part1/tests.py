...
class TestHour(unittest.TestCase):
	def test_hour(self):
		ser = pd.Series([1030.0, 1259.0], dtype='float64')
		self.assertTrue(extract_hour(ser).equals(pd.Series([10, 12], dtype='float64')))
...
class TestMinute(unittest.TestCase):
	def test_minute(self):
		ser = pd.Series([1030.0, 1259.0], dtype='float64')
		self.assertTrue(extract_mins(ser).equals(pd.Series([30, 59], dtype='float64')))
